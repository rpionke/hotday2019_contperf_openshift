...
runController() {
    ...
    echo $CONTROLLER_XMX >> /home/jenkins/neoload/bin/NeoLoadCmd.vmoptions
    sed -i "s/lg.launcher.vm.parameters=-server/lg.launcher.vm.parameters=$LOADGENERATOR_XMX -server/g" /home/jenkins/neoload/conf/agent.properties

    echo "Launching NeoLoad with following parameters "${PARAMS}

    exec /home/jenkins/neoload/bin/NeoLoadCmd ${PARAMS}
}